...
float baseControl = 0.5;
float elbowControl = 0.5;
float handControl = 0.5;
float *jointPointer = NULL;
...
void inicializa() {
    ...
    carregarTextura("textura/wood.png", madeiraTexID);
    carregarTextura("textura/azulejo.jpg", azulejoTexID);
}
...
void drawPlane(float width, float height, GLuint textureID) {
    glBindTexture(GL_TEXTURE_2D, textureID);
    glBegin(GL_POLYGON);
        glTexCoord2f(0.0,0.0); glVertex3f(-width/2, 0, -height/2);
        glTexCoord2f(0.0,2.0); glVertex3f(-width/2, 0, height/2);
        glTexCoord2f(2.0,2.0); glVertex3f(width/2, 0, height/2);
        glTexCoord2f(2.0,0.0); glVertex3f(width/2, 0, -height/2);
    glEnd();
}
...
void drawHUD() {
    ...
    float knobX = 0.2f + (*jointPointer * 0.6f); // Map slider value to X position
    glBegin(GL_QUADS);
...
void display() {
    ...
    glPushMatrix();
        ...
        drawPlane(8, 8, azulejoTexID);

        glRotatef(baseControl*2*180 - 180, 0.0, 1.0, 0.0);
        ...
        glPushMatrix();
            ...
            glColor3f(1.0, 1.0, 1.0);
            drawCylinder(16, 0.5, 2.0, madeiraTexID);
            ...
            glPushMatrix();
                ...
                glutSolidSphere(0.625, 16, 16);
                //drawCylinder(16, 0.625, 2*0.625); // Joint cylinder
            glPopMatrix();
            glColor3f(1.0, 1.0, 1.0);
        glPopMatrix();
        ...
        glRotatef(elbowControl * 180 - 90, 0.0, 0.0, 1.0);  // Rotate the top cylinder around its own axis
        ...
        glPushMatrix();
            ...
            glRotatef(handControl * 180 - 90, 0.0, 0.0, 1.0);
...
void mouse(int button, int state, int x, int y) {
    if (button == GLUT_LEFT_BUTTON) {
        if (state == GLUT_DOWN) {
            ...
            if (jointPointer != NULL &
                normX >= 0.2f && normX <= 0.8f && normY >= 0.08 && normY <= 0.14) { // Slider area
                sliderPressed = true;
...
void motion(int x, int y) {
    if (sliderPressed) {
        ...
        *jointPointer += deltaX;
        if (*jointPointer > 1.0f) *jointPointer = 1.0f;
        if (*jointPointer < 0.0f) *jointPointer = 0.0f;
...
void menu(int opcao) {
    switch (opcao) {
        case 0:
            jointPointer = &elbowControl;
            break;
        case 1:
            jointPointer = &handControl;
            break;
        case 2:
            jointPointer = &baseControl;
            break;
        default:
            break;
    }   
...
int main(int argc, char** argv) {
    ...
    glutCreateMenu(menu);
    glutAddMenuEntry("Base", 2);
    glutAddMenuEntry("Meio", 0);
    glutAddMenuEntry("Topo", 1);
